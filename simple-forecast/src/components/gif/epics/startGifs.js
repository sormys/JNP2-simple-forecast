...
import { GIFS_CHANGE_DESCRIPTION_ACTION, fetchGifs } from "../reducer"
import { interval, Observable, takeUntil, startWith } from "rxjs"
import { withLatestFrom } from "rxjs/operators"
import { gifsAPIKey } from "../../../config"

export const startGifsEpic = (action$, state$) =>
  action$.pipe(
    ...
    mergeMap((action) => {
      ...
      const interval$ = interval(30000).pipe(
        ...
        mergeMap(([_, state]) => {
          let url = `https://tenor.googleapis.com/v2/search?key=${gifsAPIKey}&q=${action.payload}&limit=1`
